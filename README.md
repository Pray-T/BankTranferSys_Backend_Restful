...
-인덱스가 필요한 이유에 관하여: 위에서 언급한 디스크 I/O의 효과적인 감소를 위해서 인덱스는 필요합니다. 인덱스는 조회 성능과 밀접적으로 관련이 있는 요소로써 조건이 있는 조회 쿼리에 맞는 레코드를 DB전체를 스캔하지 않고 빠르게 찾을 수 있습니다. 풀 테이블 스캔을 방지하고 꼭 필요한 데이터만 읽도록 설정한 인덱스는 조회성능을 향상시킵니다. 하지만 인덱스가 장점만 존재하는 것은 아닙니다, 인덱스는 읽기 성능은 향상시키지만 쓰기 성능은 저하됩니다. 그렇지만 DB에서 수행되는 대부분의 작업이 쓰기 작업이 아닌 읽기 작업임을 감안한다면 적절한 인덱스의 생성은 실보다 득이 더많은 기능입니다. 또한 DB에 저장되어 있는 레코드를 대부분 읽는 작업에서는 랜덤I/O를 사용하는 인덱스보다는 순차 I/O를 사용하는 풀 테이블 스캔이 더 빠르고 많은 레코드를 읽어 올 수 있습니다.
