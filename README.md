...
쿨다운키(Cooldown Key)또한 중복요청 방지를 위해 사용되었습니다. 같은 입금/출금계좌를 가진 이체요청이 10초 이내로 다시 시도 된다면 같은 요청의 불필요한 중복 요청으로 간주하여 HttpStatus.TOO_MANY_REQUESTS예외를 발생시키고 "방금 전 이체가 실행된 계좌 입니다, 추가 이체를 원하신다면 10초 후 다시 시도해주세요."라는 메시지와 함께 중복 요청을 차단합니다.
